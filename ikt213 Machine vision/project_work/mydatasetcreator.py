...
label = input("Name the label for these images: ")
...
while True:
    ...
    if key_input & 0xFF == ord("x"):
        label = input("Input new label name: ")

    if key_input & 0xFF == ord("c"):
        savecrop(img, landmark_normalised, label)
    ...
    def savecrop(img, landmarks, label):
        padding = 15
        img_counter = 0
        path = f"\dataset\{label}\image"
        rightPath = r"" + path
        path = f'{os.getcwd()}/{label}'
        ...
        img_name = f"{label}/{label}_{img_counter}.png"
        xmin = ymin = 1000000
        xmax = ymax = 0

        for each in landmarks:
            xmin = min(xmin, each[0])
            xmax = max(xmax, each[0])
            ymin = min(ymin, each[1])
            ymax = max(ymax, each[1])

        cropped_frame = img[ymin-padding:ymax+padding, xmin-20:xmax+15]
